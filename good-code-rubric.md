...
## Repository-specific notes for mygh

This repo is a small Cloudflare Workers product, not a monorepo. Apply the rubric with these local conventions:

- Treat `src/index.ts` as the Worker shell: route handling, request parsing, Cloudflare KV access, GitHub API fetches, redirects, and Open Graph responses live here unless the file grows enough to justify extraction.
- Treat `public/app.js` as browser-side UI orchestration: DOM events, Canvas preview rendering, and API calls belong here while the frontend remains dependency-free.
- Keep pure helpers separate from I/O-heavy route or DOM code even when they live in the same file. Prefer small named functions for URL parsing, slug handling, HTML escaping, GitHub metadata shaping, and validation.
- Preserve Cloudflare Workers compatibility. Prefer platform APIs such as `Request`, `Response`, `URL`, `Headers`, `fetch`, `crypto`, and typed `Env` bindings; avoid Node-only APIs.
- Validate external input at boundaries, especially GitHub URLs, slugs, image data URLs, crawler detection, and user/GitHub-provided text rendered into HTML.
- Keep real Cloudflare IDs, tokens, local Wrangler config, `.dev.vars`, and generated caches out of commits.
- Run `npm run check` for TypeScript changes and `npm test` when behavior or route logic changes.

Exceptions to the base rubric:

- Do not introduce `apps/`, `domains/`, or `shared/` folders unless the project grows beyond its current one-Worker shape.
- Zod/Pydantic are not installed; use explicit TypeScript types and narrow validation helpers unless adding a dependency becomes clearly worth it.
- `public/` contains plain static assets by design. Do not add a frontend framework, bundler, or dependency chain unless the user asks or the need is clear.
